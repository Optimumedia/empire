...
window.City = (() => {
  ...
  function growthOK(i, A, c, hold) {
    ...
    if (p.k === 'r') {
      ...
      if (want >= 3) { for (const sv of ['water', 'fire', 'police']) if (!i.cov[sv]) need.push(sv); if (!hasH) need.push('clinic or hospital'); if (!hasE) need.push('school or university'); if (!hold) { if (A.happiness < 75) need.push('happiness 75'); if (i.cond < 70) need.push(`condition 70 (${i.cond})`); if (i.lvs < 70) need.push('land value 70'); for (const sv of ['water', 'clinic', 'hospital', 'school', 'library', 'university', 'fire', 'police']) if (i.cov[sv] && i.eff[sv] < 0.99) need.push(sv + ' over capacity'); if (A.dem.r <= 0.2) need.push('stronger demand'); if (A.peak < POP_MAX.L3) need.push('City (1500 pop)'); } }
    } else if (p.k === 'c') {
  ...
  function mount(opts) {
    ...
    function reportHtml(c, A) {
      ...
      return `<div class="rep">
        ...
        <h4>Wealth &amp; condition</h4>${kv('Modest homes', A.wealthC[0], 'tax ×1')}${kv('Comfortable homes', A.wealthC[1], 'tax ×1.5 · land value 62+')}${kv('Affluent homes', A.wealthC[2], 'tax ×2.2 · land value 95+ with education and leisure')}${kv('Average condition', A.avgCond, 'services, health, education, leisure, land value, minus pollution and traffic')}
    ...
    function place(x, y) {
      ...
      if (p) { sel = [x, y]; renderPanel(); return false; } // occupied → inspect it
    ...
    canvas.addEventListener('pointerdown', e => {
      ...
      const r = canvas.getBoundingClientRect(); const [wx, wy] = toWorld(e.clientX - r.left, e.clientY - r.top); const [gx0, gy0] = toGrid(wx, wy); const [gx, gy] = tool === 'select' ? pickTile(wx, wy, gx0, gy0) : [gx0, gy0];
