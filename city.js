...
window.City = (() => {
  ...
  const SERVICES = {
    ...
    school: { name: 'School',      e: '🏫', grit: 150, r: 5, cap: 250, capKind: 'pop', up: 15, mile: 50, edu: 2 },
    library:{ name: 'Library',     e: '📚', grit: 90,  r: 5, cap: 300, capKind: 'pop', up: 8,  mile: 50, edu: 1 },
    university: { name: 'University', e: '🎓', grit: 400, r: 7, cap: 800, capKind: 'pop', up: 30, mile: 500, edu: 3 },
    hospital: { name: 'Hospital',  e: '⚕️', grit: 350, r: 7, cap: 1200, capKind: 'pop', up: 30, mile: 500 },
    bus:    { name: 'Bus depot',   e: '🚌', grit: 120, r: 6, up: 10, mile: 200 },
  ...
  const WHAT = {
    r: 'Homes. People move in once there is a road within 2 tiles and power. Every home has a condition (services, health, education, leisure, pollution) and a wealth tier — modest, comfortable, affluent — set by land value, education and leisure nearby. Affluent residents pay 2.2× the tax. Level 2 needs water, a clinic or school, condition 50 and Town size; level 3 needs every service, condition 70, land value 70 and City size.',
    c: 'Shops and offices. They live on customers: residents within 4 tiles, weighted by wealth. A shop needs 10 customers to open, 40 to become a mall, 120 to become an office tower — plus staff (workers available city-wide), water, and education 50 for towers. Starved of customers it closes down. Jobs 4 / 15 / 50 per level; profit sets the levy it pays.',
    i: 'Factories. The most jobs per tile (6 / 20 / 60) but they pollute 3 tiles around. They grow on workers: enough homes to staff them, water at level 2, fire cover at level 3. At city education 60 they turn into clean tech — 40% less pollution and 20% more output.',
    ...
    school: 'Education +2 for homes in reach. Unlocks level 2 homes (a clinic is the alternative). +15 land value. Serves 250 people; over capacity it works at a fraction.',
    library: 'Education +1 for homes in reach — the cheap way to lift wealth. Serves 300 people.',
    university: 'Education +3 in a wide reach. Office towers need city education 50; clean industry needs 60. +10 land value. Serves 800 people.',
    hospital: 'Health for a whole district: 1200 people, reach 7. Counts as a clinic everywhere it reaches, so towers can rise.',
    bus: 'Raises road capacity by half for every zoned tile in reach. Traffic above 100% costs happiness and shop profit.',
  ...
  const NEED_C = [0, 10, 40, 120];            // customers (weighted residents within 4 tiles) a shop needs to hold each level
  const WEALTH_MULT = [1, 1.5, 2.2], WEALTH_NAME = ['modest', 'comfortable', 'affluent'];
  ...
  function norm(c) {
    ...
    for (const k in c.tiles || {}) { const p = c.tiles[k]; if (p && KINDS.has(p.k) && !(p.gone && Date.now() - (+p.t || 0) > 30 * DAY)) out.tiles[k] = { k: p.k, lv: clamp(+p.lv || 0, 0, 3), t: Math.max(+p.t || 0, 1e12), g: +p.g || 0, d: +p.d || 0, gone: !!p.gone }; }
  ...
  function analyze(c, vit) {
    ...
    const info = {}; // per tile
    for (const k in live) { const [x, y] = k.split(',').map(Number); const p = live[k]; info[k] = { x, y, p, active: false, cov: {}, eff: {}, poll: 0, lv: 0, edu: 0, cond: 0, wealth: 0, cust: 0, perf: 0, clean: false }; if (ZONES[p.k]) info[k].active = nearRoad(x, y, 2); else if (SERVICES[p.k]) info[k].active = nearRoad(x, y, 1) || p.k === 'park'; else info[k].active = true; }
    ...
    // education: what each home's residents can reach; the city score is the average over built homes
    const eduOf = i => Math.min(1, ((i.cov.school ? 2 * i.eff.school : 0) + (i.cov.library ? i.eff.library : 0) + (i.cov.university ? 3 * i.eff.university : 0)) / 4);
    let eduSum = 0, eduN = 0; for (const k in info) { const i = info[k]; if (i.p.k !== 'r') continue; i.edu = eduOf(i); if (i.active && i.p.lv > 0) { eduSum += i.edu; eduN++; } }
    const education = eduN ? Math.round(100 * eduSum / eduN) : 0; const clean = education >= 60;
    for (const k in info) if (info[k].p.k === 'i') info[k].clean = clean;
    // pollution
    ...
    for (const k in info) { const i = info[k]; if (i.p.k === 'i' && i.p.lv > 0 && i.active) emit(i.x, i.y, ZONES.i.poll[i.p.lv] * pollMul * (clean ? 0.6 : 1), 3); if (i.p.k === 'power' && i.active) emit(i.x, i.y, SERVICES.power.poll * pollMul, SERVICES.power.pr); }
    ...
    for (const k in info) { const i = info[k]; if (!ZONES[i.p.k]) continue; if (!i.active) { zonedOff++; continue; } zc[i.p.k][i.p.lv]++; if (i.p.k === 'r') { pop += ZONES.r.pop[i.p.lv]; if (i.p.lv > 0) { rTiles++; let n = 0; for (const s of ['power', 'water', 'fire', 'police']) if (i.cov[s]) n += i.eff[s]; n += Math.max(i.cov.clinic ? i.eff.clinic : 0, i.cov.hospital ? i.eff.hospital : 0); n += Math.max(i.cov.school ? i.eff.school : 0, i.cov.library ? i.eff.library : 0, i.cov.university ? i.eff.university : 0); rCov += n / 6; if (i.cov.park || i.cov.stadium) rPark++; rPoll += i.poll; } else vacantR++; } else if (i.p.k === 'c') jobsC += ZONES.c.jobs[i.p.lv]; else jobsI += Math.round(ZONES.i.jobs[i.p.lv] * (clean ? 1.2 : 1)); }
    ...
    // traffic: trips against road capacity; bus depots add half a lane for every zoned tile they reach
    let zonedN = 0, busN = 0; for (const k in info) if (ZONES[info[k].p.k] && info[k].active) { zonedN++; if (info[k].cov.bus) busN++; }
    const trips = workforce + 0.3 * jobs, roadCap = conn.size * 40 * (1 + 0.5 * (zonedN ? busN / zonedN : 0)); const congestion = roadCap ? trips / roadCap : 0; const trafficPen = congestion > 1 ? Math.min(15, Math.round(10 * (congestion - 1))) : 0;
    const staffing = jobs > 0 ? clamp(workforce / jobs, 0, 1) : 1;
    const happiness = clamp(Math.round(50 + 25 * avgCov + 10 * parkShare - 0.1 * avgPoll - 3 * (tax - 7) + 20 * (clamp(vit.family == null ? 0.5 : vit.family, 0, 1) - 0.5) + fest - trafficPen), 0, 100);
    // land value
    const lms = []; for (const k in live) { const d = DECOR[live[k].k]; if (d && d.rank) { const [x, y] = k.split(',').map(Number); lms.push({ x, y, r: d.r, b: d.lv }); } }
    for (const k in info) { const i = info[k]; let lv = 30; for (const lm of lms) if (Math.max(Math.abs(i.x - lm.x), Math.abs(i.y - lm.y)) <= lm.r) lv += lm.b; if (i.cov.park) lv += 20; if (i.cov.school || i.cov.library || i.cov.university || i.cov.clinic || i.cov.hospital) lv += 15; if (i.cov.university) lv += 10; if (i.cov.fire && i.cov.police) lv += 10; let l3 = 0, adjI = false; for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) { const o = at(i.x + dx, i.y + dy); if (o && ZONES[o.k] && o.lv === 3) l3++; if (o && o.k === 'i') adjI = true; } lv += 5 * l3 - i.poll - (adjI ? 10 : 0); for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) { const o = at(i.x + dx, i.y + dy); if (o && o.k === 'plaza') i.cov.plaza = true; } if (i.cov.plaza) lv += 5; if (i.cov.stadium) lv += 10; i.lv = clamp(Math.round(lv), 0, 100); }
    ...
    // homes: condition and wealth · shops: customers and profit · industry: staffing
    const wealthC = [0, 0, 0]; let rTax = 0, condSum = 0, condN = 0;
    for (const k in info) { const i = info[k]; if (i.p.k !== 'r') continue; const leisure = !!(i.cov.park || i.cov.plaza || i.cov.stadium); let svcN = 0; for (const sv of ['power', 'water', 'fire', 'police']) if (i.cov[sv]) svcN += i.eff[sv]; const health = Math.max(i.cov.clinic ? i.eff.clinic : 0, i.cov.hospital ? i.eff.hospital : 0);
      i.condF = { base: 20, services: 10 * svcN, health: 15 * health, education: 15 * i.edu, leisure: leisure ? 10 : 0, 'land value': 0.2 * i.lvs, pollution: -i.poll, traffic: -trafficPen / 2 }; i.health = health;
      i.cond = clamp(Math.round(20 + 10 * svcN + 15 * health + 15 * i.edu + 10 * (leisure ? 1 : 0) + 0.2 * i.lvs - i.poll - trafficPen / 2), 0, 100);
      const ws = i.lvs + 25 * i.edu + (leisure ? 12 : 0); i.ws = ws; i.wealth = ws >= 95 ? 2 : ws >= 62 ? 1 : 0;
      if (i.active && i.p.lv > 0) { wealthC[i.wealth]++; rTax += ZONES.r.pop[i.p.lv] * WEALTH_MULT[i.wealth]; condSum += i.cond; condN++; } }
    let custSum = 0, custN = 0, perfSum = 0;
    for (const k in info) { const i = info[k]; if (i.p.k === 'c') { let cust = 0; for (const kk in info) { const o = info[kk]; if (o.p.k === 'r' && o.active && Math.max(Math.abs(o.x - i.x), Math.abs(o.y - i.y)) <= 4) cust += ZONES.r.pop[o.p.lv] * (1 + 0.5 * o.wealth); } i.cust = Math.round(cust); i.hiTech = education >= 50;
        i.perfF = { customers: 0.55 * Math.min(1, i.cust / NEED_C[Math.min(3, i.p.lv + 1)]), staffing: 0.25 * staffing, happiness: 0.2 * happiness / 100, traffic: congestion > 1 ? -0.15 : 0 };
        i.perf = clamp(0.55 * Math.min(1, i.cust / NEED_C[Math.min(3, i.p.lv + 1)]) + 0.25 * staffing + 0.2 * happiness / 100 - (congestion > 1 ? 0.15 : 0), 0, 1); if (i.active && i.p.lv > 0) { custSum += i.cust; custN++; perfSum += i.perf; } }
      else if (i.p.k === 'i') i.perf = clamp(0.6 * staffing + 0.4 * (dem.i > 0 ? 1 : 0.5), 0, 1); }
    const avgCond = condN ? Math.round(condSum / condN) : 0, avgPerf = custN ? perfSum / custN : 0;
    // budget (per day)
    let avgLV = 0, nz = 0; for (const k in info) if (ZONES[info[k].p.k]) { avgLV += info[k].lvs; nz++; } avgLV = nz ? avgLV / nz : 30;
    const lvF = 0.75 + 0.5 * avgLV / 100; const wealthMult = pop ? rTax / pop : 1; const incomeR = rTax * 0.5 * (tax / 7) * lvF * M.business; let incomeJ = 0; for (const k in info) { const i = info[k]; if (!i.active) continue; if (i.p.k === 'c') incomeJ += ZONES.c.jobs[i.p.lv] * 0.1 * (0.5 + i.perf); else if (i.p.k === 'i') incomeJ += ZONES.i.jobs[i.p.lv] * (clean ? 1.2 : 1) * 0.1; } const income = incomeR + incomeJ;
    ...
    const hapF = { base: 50, services: 25 * avgCov, parks: 10 * parkShare, pollution: -0.1 * avgPoll, tax: -3 * (tax - 7), family: 20 * (clamp(vit.family == null ? 0.5 : vit.family, 0, 1) - 0.5), festival: fest, traffic: -trafficPen };
    ...
    return { M, info, svc, conn, hall, pop, jobs, jobsC, jobsI, workforce, happiness, dem, income, upkeep, avgCov, parkShare, avgPoll, avgLV, vacantR, mile, next, peak, tax, unemployed: Math.max(0, workforce - jobs), incomeR, incomeJ, lvF, upkeepBy, hapF, zc, zonedOff, pollMul, education, clean, congestion, trips, roadCap, staffing, wealthC, wealthMult, avgCond, avgPerf, custAvg: custN ? Math.round(custSum / custN) : 0 };
  }
  ...
  function growthOK(i, A, c, hold) {
    const p = i.p, need = []; const want = hold ? p.lv : p.lv + 1; const hasH = i.cov.clinic || i.cov.hospital, hasE = i.cov.school || i.cov.library || i.cov.university;
    if (want >= 1) { if (!i.active) need.push('road within 2 tiles'); if (!i.cov.power) need.push('power'); else if (!hold && i.eff.power < 0.5) need.push('power over capacity'); }
    if (!hold && want >= 1 && A.dem[p.k] <= 0 && A.pop > 0) need.push('demand');
    if (p.k === 'r') {
      if (want >= 2) { if (!i.cov.water) need.push('water'); if (!(hasH || hasE)) need.push('clinic or school'); if (!hold) { if (A.happiness < 60) need.push('happiness 60'); if (i.cond < 50) need.push(`condition 50 (${i.cond})`); if (A.peak < POP_MAX.L2) need.push('Town (500 pop)'); } }
      if (want >= 3) { for (const sv of ['water', 'fire', 'police']) if (!i.cov[sv]) need.push(sv); if (!hasH) need.push('clinic or hospital'); if (!hasE) need.push('school or university'); if (!hold) { if (A.happiness < 75) need.push('happiness 75'); if (i.cond < 70) need.push(`condition 70 (${i.cond})`); if (i.lvs < 70) need.push('land value 70'); for (const sv of ['power', 'water', 'clinic', 'hospital', 'school', 'library', 'university', 'fire', 'police']) if (i.cov[sv] && i.eff[sv] < 0.99) need.push(sv + ' over capacity'); if (A.dem.r <= 0.2) need.push('stronger demand'); if (A.peak < POP_MAX.L3) need.push('City (1500 pop)'); } }
    } else if (p.k === 'c') {
      if (hold) { if (want >= 1 && i.cust < 0.4 * NEED_C[want]) need.push('customers'); if (want >= 2 && A.staffing < 0.3) need.push('staff'); }
      else { if (want >= 1 && i.cust < NEED_C[want]) need.push(`${NEED_C[want]} customers in reach (${i.cust})`); if (want >= 2) { if (!i.cov.water) need.push('water'); if (A.staffing < 0.6) need.push('staff — more homes'); if (A.peak < POP_MAX.L2) need.push('Town (500 pop)'); } if (want >= 3) { if (A.education < 50) need.push(`education 50 (${A.education})`); if (i.perf < 0.75) need.push(`profit index 0.75 (${i.perf.toFixed(2)})`); if (A.peak < POP_MAX.L3) need.push('City (1500 pop)'); } }
    } else {
      if (hold) { if (want >= 2 && A.staffing < 0.3) need.push('workers'); }
      else { if (want >= 2) { if (!i.cov.water) need.push('water'); if (A.staffing < 0.5) need.push('workers — more homes'); if (A.peak < POP_MAX.L2) need.push('Town (500 pop)'); } if (want >= 3) { if (A.staffing < 0.7) need.push('workers'); if (!i.cov.fire) need.push('fire'); if (A.peak < POP_MAX.L3) need.push('City (1500 pop)'); } }
    }
  ...
  function advisor(c, A) {
    ...
    if (A.congestion > 1) return `Roads are at ${Math.round(A.congestion * 100)}% — add roads, or a bus depot lifts capacity by half in its reach.`;
    if (A.pop >= 100 && A.education < 20) return 'Nobody is educated. A library is cheap and starts lifting wealth — and tax.';
    const starving = zones.filter(i => i.p.k === 'c' && i.p.lv > 0 && i.cust < 0.4 * NEED_C[i.p.lv]).length; if (starving) return `${starving} shop${starving > 1 ? 's have' : ' has'} no customers — homes must be within 4 tiles.`;
    if (A.jobs > 0 && A.staffing < 0.5) return 'Businesses can’t find staff. Zone more homes before more jobs.';
  ...
  function mount(opts) {
    ...
    let cars = [], peds = [], smoke = [], fx = [], lastA = null, awayShown = false;
    const now = () => Date.now();
    ...
    function drawStructure(x, y, p, night, i) {
      ...
      if (ZONES[p.k]) {
        ...
        if (p.k === 'r') {
          const wl = i && i.wealth ? i.wealth : 0; const lawnS = 35 + wl * 10;
          if (lv === 1) { const w = 0.55 + r() * 0.15; const hue = wl === 2 ? 40 : wl === 1 ? 28 + r() * 10 : 25 + r() * 20; const h = 12 + r() * 4 + wl * 2; const cc = corners(x, y, 1, 1); poly([cc.b, cc.r, cc.f, cc.l], hsl(120, lawnS, night ? 24 : 52)); const c = box(x, y, w, w, h, hue, [45 + r() * 10, 26, 8][wl], [58 + r() * 8, 74, 90][wl], { win: { rng: r, fp: 10, cols: 2 }, night }); roof(c, h, 8 + r() * 3, wl === 2 ? 42 : r() < 0.5 ? 8 : 200, wl === 2 ? 55 : 35, wl === 2 ? 52 : 42); g.fillStyle = hsl(28, 40, 28); g.fillRect(c.f[0] - 7, c.f[1] - 8, 4, 8);
            if (wl === 2) { ell(cx + 17, cy + 6, 9, 4.5, hsl(200, 70, 62), 'rgba(255,255,255,.8)', 1.2); ell(cx + 17, cy + 6, 6, 3, hsl(195, 85, 72)); tree(cx - 19, cy + 3, 0.6, 135); tree(cx - 11, cy + 8, 0.5, 135); }
            else if (wl === 1) { tree(cx - 18, cy + 4, 0.7, 125 + r() * 15); g.fillStyle = hsl(120, 45, 34); g.fillRect(cx + 8, cy + 2, 12, 2.5); }
            else if (r() < 0.6) tree(cx - 18, cy + 4, 0.7, 120 + r() * 20); }
          else if (lv === 2) { const h = 26 + r() * 14 + wl * 3; const c = box(x, y, 0.85, 0.85, h, wl === 2 ? 40 : 30 + r() * 15, [40, 32, 10][wl], [60, 70, 84][wl], { win: { rng: r, fp: 9, cols: 3 }, night, glass: wl === 2 }); if (wl < 2) { g.fillStyle = 'rgba(0,0,0,.18)'; for (let f = 1; f < Math.floor(h / 9); f++) g.fillRect(c.l[0] + (c.f[0] - c.l[0]) * 0.15, c.l[1] + (c.f[1] - c.l[1]) * 0.15 - f * 9 - 3, 8, 1.5); g.fillStyle = hsl(30, 30, 40); g.fillRect(cx + 4, cy - h - 12, 5, 8); }
            else { poly([up(c.l, h - 3), up(c.f, h - 3), up(c.f, h - 5.5), up(c.l, h - 5.5)], hsl(45, 85, 58)); poly([up(c.f, h - 3), up(c.r, h - 3), up(c.r, h - 5.5), up(c.f, h - 5.5)], hsl(45, 85, 48)); tree(cx + 2, cy - h - 4, 0.45, 130); } }
          else { const h = 60 + r() * 30 + wl * 8; const c = box(x, y, 0.8, 0.8, h, wl === 2 ? 215 : wl === 1 ? 205 : 35 + r() * 10, [30, 28, 40][wl], [62, 58, 64][wl], { win: { rng: r, fp: 8, cols: 3 }, night, glass: wl >= 1, roof: wl >= 1 ? hsl(210, 20, 45) : undefined });
            if (wl === 2) { ell(cx, cy - h - 5, 11, 5.5, hsl(45, 85, 58), 'rgba(0,0,0,.2)'); g.strokeStyle = hsl(45, 85, 70); g.lineWidth = 1.5; g.beginPath(); g.moveTo(cx, cy - h - 6); g.lineTo(cx, cy - h - 22); g.stroke(); }
            else { cyl(cx + 8, cy - h - 2, 4, 8, 30, 30, 45); g.fillStyle = hsl(30, 20, 35); g.fillRect(cx + 6, cy - h + 6, 1.5, 6); g.fillRect(cx + 10, cy - h + 6, 1.5, 6); } }
        } else if (p.k === 'c') {
          ...
          else { const h = 76 + r() * 40 + (i && i.hiTech ? 18 : 0); const c = box(x, y, 0.75, 0.75, h, 205 + r() * 25, 35, 52, { win: { rng: r, fp: 7, cols: 4, ribbon: true }, night, glass: true, roof: hsl(210, 20, 40) }); box(x, y, 0.4, 0.4, h + 12, 205, 30, 50, { roof: hsl(210, 20, 42) }); g.strokeStyle = 'rgba(255,255,255,.75)'; g.lineWidth = 1.5; g.beginPath(); g.moveTo(cx, cy - h - 12); g.lineTo(cx, cy - h - 28); g.stroke(); ell(cx, cy - h - 29, 2, 2, '#ff5252'); }
        } else { // industrial — clean tech at city education 60 swaps brick and stacks for white halls and solar roofs
          const cl = !!(i && i.clean); const IH = cl ? 205 : 20, IS = cl ? 18 : 12, IL = cl ? 72 : 46;
          const solar = (c, h) => { g.fillStyle = hsl(215, 60, 38); for (let k = 0; k < 3; k++) { const t0 = 0.12 + k * 0.28, t1 = t0 + 0.2; poly([[c.b[0] + (c.r[0] - c.b[0]) * t0, c.b[1] + (c.r[1] - c.b[1]) * t0 - h], [c.b[0] + (c.r[0] - c.b[0]) * t1, c.b[1] + (c.r[1] - c.b[1]) * t1 - h], [c.l[0] + (c.f[0] - c.l[0]) * t1, c.l[1] + (c.f[1] - c.l[1]) * t1 - h], [c.l[0] + (c.f[0] - c.l[0]) * t0, c.l[1] + (c.f[1] - c.l[1]) * t0 - h]], hsl(215, 60, 42)); } };
          if (lv === 1) { const h = 14 + r() * 4; const c = box(x, y, 0.85, 0.7, h, IH, IS, IL, { roof: hsl(IH, IS - 2, IL - 6) }); if (cl) solar(c, h); else cyl(c.b[0] + 8, c.b[1] + 6, 3, 18, 0, 5, 40); g.fillStyle = hsl(25, 30, cl ? 55 : 35); g.fillRect(c.f[0] - 10, c.f[1] - 10, 8, 10); }
          else if (lv === 2) { const h = 20 + r() * 6; const c = box(x, y, 0.95, 0.9, h, IH + 5, IS + 3, IL - 2, { roof: hsl(IH, IS, IL - 6) }); if (cl) solar(c, h); else { for (let k = 0; k < 3; k++) { poly([up(c.b, h), up(c.r, h), up([c.r[0], c.r[1]], h + 5), up([c.b[0], c.b[1]], h + 5)], hsl(25, 12, 50)); } cyl(c.b[0] + 6, c.b[1] + 8, 3, 24, 0, 5, 42); cyl(c.b[0] + 16, c.b[1] + 13, 3, 20, 0, 5, 42); } g.strokeStyle = hsl(30, 20, 55); g.lineWidth = 2; g.beginPath(); g.moveTo(c.l[0] + 4, c.l[1] - 6); g.lineTo(c.f[0] - 6, c.f[1] - 10); g.stroke(); }
          else { const h = 26 + r() * 8; const c = box(x, y, 1.0, 0.95, h, IH, IS + 3, IL - 6, { roof: hsl(IH, IS, IL - 12) }); if (cl) { solar(c, h); box(x, y, 0.3, 0.3, h + 10, 205, 20, 78, { roof: hsl(205, 20, 70) }); } else { cyl(c.l[0] + 10, c.l[1] - 2, 7, 14, 200, 10, 55); cyl(c.f[0] - 12, c.f[1] - 8, 6, 12, 200, 10, 55); cyl(c.b[0] + 4, c.b[1] + 10, 4, 34, 0, 5, 38); cyl(c.b[0] + 14, c.b[1] + 14, 4, 30, 0, 5, 38); ell(c.b[0] + 4, c.b[1] + 10 - 35, 1.8, 1.8, '#ff5252'); } }
        }
        return;
      }
      switch (p.k) {
        ...
        case 'library': { const c = box(x, y, 0.85, 0.75, 14, 28, 35, 64, { win: { rng: r, fp: 9, cols: 3 }, night, roof: hsl(28, 30, 46) }); g.fillStyle = '#f3ede2'; for (let k = 0; k < 3; k++) { const t = 0.2 + k * 0.28; g.fillRect(c.l[0] + (c.f[0] - c.l[0]) * t - 1, c.l[1] + (c.f[1] - c.l[1]) * t - 13, 2.2, 12); } label('📚', cx + 10, cy - 20, 9); break; }
        case 'university': { const c = box(x, y, 1.0, 0.9, 24, 350, 22, 58, { win: { rng: r, fp: 9, cols: 4 }, night, roof: hsl(350, 20, 40) }); cyl(cx - 2, cy - 24, 5, 16, 350, 15, 62); ell(cx - 2, cy - 42, 6, 3.5, hsl(45, 60, 60), 'rgba(0,0,0,.2)'); g.fillStyle = '#fff'; g.fillRect(cx - 4, cy - 34, 4, 4); label('🎓', c.f[0] - 6, c.f[1] - 30, 10); break; }
        case 'hospital': { const c = box(x, y, 0.95, 0.9, 30, 0, 0, 90, { win: { rng: r, fp: 9, cols: 4 }, night }); g.fillStyle = '#d9342b'; g.fillRect(cx - 2, cy - 46, 4, 12); g.fillRect(cx - 6, cy - 42, 12, 4); ell(c.b[0] + 14, c.b[1] + 4 - 30, 7, 3.5, hsl(0, 0, 80), '#d9342b', 1); g.fillStyle = '#d9342b'; g.font = 'bold 7px sans-serif'; g.textAlign = 'center'; g.fillText('H', c.b[0] + 14, c.b[1] + 6 - 30); break; }
        case 'bus': { const c = box(x, y, 0.9, 0.8, 12, 50, 55, 58, { roof: hsl(50, 40, 45) }); g.fillStyle = hsl(45, 90, 55); g.fillRect(c.f[0] - 16, c.f[1] - 9, 14, 6); g.fillStyle = '#333'; g.fillRect(c.f[0] - 14, c.f[1] - 8, 3, 3); g.fillRect(c.f[0] - 9, c.f[1] - 8, 3, 3); g.fillRect(c.f[0] - 4, c.f[1] - 8, 3, 3); label('🚌', cx, cy - 20, 10); break; }
    ...
    function drawFrame() {
      ...
      if (tool === 'service' && sub && SERVICES[sub] && SERVICES[sub].r && hover) { const R = (sub === 'park' || sub === 'stadium') && A ? Math.max(2, Math.round(SERVICES[sub].r * A.M.body)) : SERVICES[sub].r; for (let dx = -R; dx <= R; dx++) for (let dy = -R; dy <= R; dy++) { const x = hover[0] + dx, y = hover[1] + dy; if (x < 0 || y < 0 || x >= W || y >= W) continue; const cc = corners(x, y, 1, 1); poly([cc.b, cc.r, cc.f, cc.l], 'rgba(62,124,177,.22)'); } }
      // selection / hover
    ...
    function step(dt) {
      ...
      const wantP = Math.min(24, Math.floor(A.pop / 6)) * (isNight() ? 0.3 : 1);
      while (peds.length < wantP && roads.length) { const r0 = roads[Math.floor(Math.random() * roads.length)]; peds.push({ x: r0[0] + 0.5, y: r0[1] + 0.5, tx: r0[0], ty: r0[1], dx: 0, dy: 0, hue: Math.floor(Math.random() * 360), sp: 0.22 + Math.random() * 0.12, side: Math.random() < 0.5 ? 1 : -1 }); }
      peds.length = Math.min(peds.length, Math.floor(wantP));
      for (const car of [...cars, ...peds]) {
      ...
      if (Math.random() < dt * 3) for (const k in A.info) { const i = A.info[k]; if ((i.p.k === 'i' && i.p.lv >= 2 && !i.clean) || i.p.k === 'power') if (Math.random() < 0.3) { const [sx, sy] = toScreen(i.x, i.y); smoke.push({ x: sx + (i.p.k === 'power' ? -26 : -14), y: sy + THH - (i.p.k === 'power' ? 40 : 30), age: 0 }); } }
    ...
    function tick() {
      ...
      if (ev.grew || ev.fell || ev.touched) { dirtyB = true; dirtyG = true; }
    ...
    function renderHud() {
      ...
      hud.innerHTML = `
        ...
        <div class="ctile" data-rep="1"><div class="k">Bucks ${trendArrow(c, 'net')}</div><div class="v">${fmt(Math.max(0, bucks(c)))}</div><div class="s">${net >= 0 ? '+' : ''}${fmt(net)}/day · +${fmt(A.income)} −${fmt(A.upkeep)}</div></div>
        ...
        <div class="ctile wide" data-rep="1"><div class="k">Happiness ${A.happiness} ${trendArrow(c, 'hap')} · Edu ${A.education} · Traffic ${Math.round(A.congestion * 100)}% · Demand</div><div class="rcis"><span>R</span>${bar(A.dem.r, '#C9491F')}<span>C</span>${bar(A.dem.c, '#3E7CB1')}<span>I</span>${bar(A.dem.i, '#8a7a4a')}</div></div>
        <button class="collect${pend >= 1 ? ' ready' : ''}" id="city-collect" ${(pend >= 1 || !c.lastTax.t) ? '' : 'disabled'}>${c.lastTax.t ? `Collect ${fmt(pend)} Bucks in taxes` : 'Open the treasury'}<span class="s">12 h cap</span></button>`;
      hud.querySelectorAll('[data-rep]').forEach(el => el.addEventListener('click', () => { tool = 'report'; ptab = 'report'; sub = null; renderPanel(); panel.scrollIntoView({ behavior: 'smooth', block: 'start' }); }));
      hud.querySelector('#city-collect').addEventListener('click', () => { const S2 = get(); const c2 = S2.city; const t2 = now(); const p = Math.floor(taxPending(c2, lastA || analyze(c2, S2.vitality), t2)); (c2.ledger[deviceId] ||= { earned: 0, spent: 0 }).earned += p; c2.lastTax.t = t2; commit(); if (p > 0) { toast(`+${fmt(p)} Bucks`); burst(Math.min(14, 3 + Math.floor(p / 50)), `+${fmt(p)}`); } else toast('Treasury open — taxes accrue from now'); renderHud(); });
    }
    ...
    function reportHtml(c, A) {
      ...
      for (const [t, u] of Object.entries(A.upkeepBy)) if (u.cap && u.load > u.cap) issues.push(`${SERVICES[t].name}${u.n > 1 ? ` ×${u.n}` : ''} over capacity (${u.load}/${u.cap}) — build another or homes stall.`);
      ...
      const hf = A.hapF; const fac = [['Base', hf.base], ['Services in reach', hf.services], ['Parks share', hf.parks], ['Pollution on homes', hf.pollution], [`Tax ${A.tax}% (7% is neutral)`, hf.tax], ['Your Family stat', hf.family], ['Festival', hf.festival], ['Traffic', hf.traffic]];
      ...
      return `<div class="rep">
        ...
        <h4>Budget · per day</h4>${kv('Residential tax', '+' + fmt(A.incomeR), `${fmt(A.pop)} people × wealth ${A.wealthMult.toFixed(2)} × 0.5 × tax ${A.tax}/7 × land value ${A.lvF.toFixed(2)} × Business ${A.M.business.toFixed(2)}`)}${kv('Business levy', '+' + fmt(A.incomeJ), `${A.jobs} jobs × 0.1 × profit${A.clean ? ' · clean tech +20%' : ''}`)}${Object.entries(A.upkeepBy).map(([t, u]) => kv(`${SERVICES[t].name}${u.n > 1 ? ` ×${u.n}` : ''}`, '−' + fmt(u.up))).join('')}${kv('<b>Net</b>', `<span style="color:${net >= 0 ? 'var(--go)' : 'var(--body)'}">${net >= 0 ? '+' : ''}${fmt(net)}</span>`)}<div class="chint">Taxes accrue for 12 h, then wait for you to collect. Bucks buy decor and festivals only.</div>
        ...
        <h4>Education ${A.education}</h4>${kv('Homes with a school', Object.values(A.info).filter(i => i.p.k === 'r' && i.cov.school).length, 'education +2')}${kv('… a library', Object.values(A.info).filter(i => i.p.k === 'r' && i.cov.library).length, '+1')}${kv('… a university', Object.values(A.info).filter(i => i.p.k === 'r' && i.cov.university).length, '+3')}${kv('Unlocks', A.education >= 60 ? 'office towers · clean industry' : A.education >= 50 ? 'office towers · clean industry at 60' : 'office towers at 50 · clean industry at 60')}
        <h4>Wealth &amp; condition</h4>${kv('Modest homes', A.wealthC[0], 'tax ×1')}${kv('Comfortable homes', A.wealthC[1], 'tax ×1.5 · wealth score 62+')}${kv('Affluent homes', A.wealthC[2], 'tax ×2.2 · wealth score 95+ (land value + education + leisure)')}${kv('Average condition', A.avgCond, 'services, health, education, leisure, land value, minus pollution and traffic')}
        <h4>Commerce</h4>${kv('Customers per shop', A.custAvg, 'residents within 4 tiles, weighted by wealth · needs 10 / 40 / 120')}${kv('Staffing', `${Math.round(A.staffing * 100)}%`, 'workers available for the jobs')}${kv('Profit index', A.avgPerf.toFixed(2), 'customers, staff, happiness, traffic · 0.75 opens towers')}
        <h4>Traffic ${Math.round(A.congestion * 100)}%</h4>${kv('Trips', fmt(Math.round(A.trips)), 'workers + a third of jobs')}${kv('Road capacity', fmt(Math.round(A.roadCap)), '40 per connected road tile, +50% under bus cover')}
        <h4>Services</h4>${svcRows}
        <h4>Land</h4>${kv('Average land value', Math.round(A.avgLV), 'parks +20 · school/library/clinic/hospital +15 · university +10 · fire+police +10 · landmarks · towers next door +5 · pollution and industry −')}${kv('Pollution on homes', Math.round(A.avgPoll), `Health stat ×${A.pollMul.toFixed(2)} on every source`)}${kv('Peak population', fmt(A.peak), A.next ? `${A.next[1]} at ${fmt(A.next[0])}` : 'top tier')}
    ...
    function guideHtml(A, S) {
      ...
      const growth = ['<b>Homes</b> · L1: road within 2, power, demand. L2: water, clinic or school, condition 50, happiness 60, Town. L3: every service in reach and under capacity, condition 70, land value 70, happiness 75, City.', '<b>Shops</b> · open at 10 customers in reach, mall at 40 with staff and water, office tower at 120 with profit index 0.75 and city education 50. Below 40% of their customers they close.', '<b>Industry</b> · L2 needs water and 50% staffing, L3 needs 70% and fire cover. Education 60 turns it clean: −40% pollution, +20% jobs.', '<b>Wealth</b> · land value, education and leisure make homes comfortable (tax ×1.5) then affluent (×2.2). Family speeds homes, Business speeds shops. Homes decline 48 h after losing road, power, water or a service their level needs; shops when customers or staff vanish; industry when workers vanish.'].map(t => `<div class="chint">• ${t}</div>`).join('');
      ...
      return `<div class="rep"><h4>Zones</h4>${z}<h4>How buildings grow</h4>${growth}<h4>Services · Grit</h4>${sv}<h4>Decor &amp; landmarks · Bucks</h4>${dc}<h4>Land</h4><div class="chint">Ring 1 · ${fmt(expandCost({ ring: { v: 0 } }))} Grit at 50 people · Ring 2 · ${fmt(expandCost({ ring: { v: 1 } }))} at 200 · Ring 3 · ${fmt(expandCost({ ring: { v: 2 } }))} at 500. Roads ${ROAD_GRIT} Grit a tile.</div><h4>Milestones · peak population</h4>${miles}<h4>Taxes</h4><div class="chint">Each resident pays 0.5 Bucks a day at 7%; the rate scales it, land value swings it from −25% to +25% (50 is neutral), and your Business stat multiplies it. Jobs add 0.1 each. Every point above 7% costs 3 happiness and cools demand; every point below does the opposite. Taxes accrue 12 hours, then wait to be collected.</div></div>`;
    }
    // ---- Panel: tools + info ----
    ...
    const PTABS = [['build', '🏗️', 'Build'], ['manage', '🏛️', 'Manage'], ['report', '📈', 'Report'], ['guide', '📖', 'Guide']];
    const BUILD_IDS = new Set(['road', 'r', 'c', 'i', 'service', 'decor', 'bulldoze']), MANAGE_IDS = new Set(['select', 'land', 'view']);
    let ptab = 'manage';
    const hrs = ms => ms > HOUR ? Math.ceil(ms / HOUR) + ' h' : Math.ceil(ms / 60000) + ' min';
    const signedF = v => (v >= 0 ? '+' : '−') + Math.abs(v).toFixed(v % 1 ? 1 : 0);
    function detailHtml(x, y, c, A) {
      const p = tileAt(c, x, y); const i = A.info[kkey(x, y)]; const peak = A.peak; const li = arr => arr.length ? `<ul class="imp">${arr.map(t => `<li>${t}</li>`).join('')}</ul>` : '<div class="chint">Nothing to fix here.</div>';
      if (!owned(c, x, y)) return `<div class="chint">Outside your land. Open <b>Land &amp; tax</b> to expand${c.ring.v < 3 ? ` — ${fmt(expandCost(c))} Grit${peak < (LAND_GATE[c.ring.v] || 0) ? `, from ${fmt(LAND_GATE[c.ring.v])} people` : ''}` : ''}.</div>`;
      if (!p) return `<div class="chint">Empty land at ${x},${y}. Anything you place here needs a road within 2 tiles (services: next door).</div>`;
      if (ZONES[p.k]) {
        const z = ZONES[p.k]; const ok = growthOK(i, A, c); const hold = growthOK(i, A, c, true); const tm = p.lv < 3 ? GROW_H[p.lv] * HOUR / (p.k === 'r' ? A.M.family : p.k === 'c' ? A.M.business : 1) : 0; const left = p.g ? Math.max(0, tm - (now() - p.g)) : tm;
        const state = p.lv >= 3 ? 'Maxed out.' : !i.active ? 'No connected road within 2 tiles — nothing happens here.' : ok.ok ? `Growing to level ${p.lv + 1} in ${hrs(left)}.` : `Stuck at level ${p.lv}.`;
        const improve = []; if (p.lv < 3 && i.active) for (const n of ok.need) improve.push(`Level ${p.lv + 1} needs <b>${n}</b>.`); if (!i.active) improve.push('Draw a road within 2 tiles of it.');
        if (p.lv > 0 && !hold.ok) improve.unshift(`<b>At risk:</b> losing ${hold.need.join(', ')} — it declines in ${p.d ? hrs(DECLINE_H * HOUR - (now() - p.d)) : '48 h'}.`);
        let head = '', econ = '', cond = '';
        if (p.k === 'r') {
          const tax = z.pop[p.lv] * WEALTH_MULT[i.wealth] * 0.5 * (A.tax / 7) * A.lvF * A.M.business; const nextW = i.wealth === 0 ? 62 : i.wealth === 1 ? 95 : null;
          head = `<div class="csel"><span class="e">${z.e}</span><div><b>${z.name} · ${p.lv === 0 ? (p.g ? 'under construction' : 'vacant lot') : `level ${p.lv}`}</b><div class="s">${state}</div></div></div>`;
          econ = `<h4>Economy</h4>${kv('Residents', z.pop[p.lv])}${kv('Tax paid', `${tax.toFixed(1)} / day`, `× ${WEALTH_MULT[i.wealth]} for ${WEALTH_NAME[i.wealth]} residents`)}${kv('Wealth', WEALTH_NAME[i.wealth], nextW ? `score ${Math.round(i.ws)} — next tier at ${nextW}` : 'top tier')}${kv('Land value here', i.lvs)}${kv('Education', `${Math.round(i.edu * 100)}%`, 'school +2 · library +1 · university +3, of 4')}`;
          cond = `<h4>Condition ${i.cond}</h4>${Object.entries(i.condF).map(([k, v]) => kv(k, signedF(v))).join('')}${kv('Health cover', `${Math.round(i.health * 100)}%`, 'clinic or hospital in reach')}${kv('Pollution here', i.poll)}`;
          if (nextW) improve.push(`Wealth score ${Math.round(i.ws)} → ${nextW}: raise land value (park, school or library, landmark in reach), education (library, school, university) and leisure (park, plaza, stadium) around it.`);
          if (i.poll > 5) improve.push(`Pollution ${i.poll} on this home — move industry away or put a park between.`);
          if (i.health < 1 && i.p.lv >= 1) improve.push(i.health === 0 ? 'No clinic or hospital in reach.' : 'Clinic over capacity — a hospital covers 1200 people.');
        } else if (p.k === 'c') {
          const needN = NEED_C[Math.min(3, p.lv + 1)]; const levy = z.jobs[p.lv] * 0.1 * (0.5 + i.perf);
          head = `<div class="csel"><span class="e">${z.e}</span><div><b>${z.name} · ${p.lv === 0 ? (p.g ? 'under construction' : 'vacant lot') : p.lv === 1 ? 'shop' : p.lv === 2 ? 'mall' : 'office tower'}</b><div class="s">${state}</div></div></div>`;
          econ = `<h4>Economy</h4>${kv('Customers', `${i.cust} / ${needN}`, 'residents within 4 tiles, wealthy ones count more')}${kv('Staffing', `${Math.round(A.staffing * 100)}%`, 'workers available city-wide')}${kv('Jobs', z.jobs[p.lv])}${kv('Levy paid', `${levy.toFixed(1)} / day`, 'jobs × 0.1 × (0.5 + profit)')}`;
          cond = `<h4>Profit index ${i.perf.toFixed(2)}</h4>${Object.entries(i.perfF).map(([k, v]) => kv(k, signedF(v))).join('')}`;
          if (i.cust < needN) improve.push(`Customers ${i.cust} of ${needN}: zone homes within 4 tiles, or make the nearby homes wealthier.`);
          if (A.staffing < 0.6) improve.push('Staff short — more homes across the city.'); if (A.congestion > 1) improve.push('Traffic is cutting profit — roads or a bus depot.');
        } else {
          const jobs = Math.round(z.jobs[p.lv] * (i.clean ? 1.2 : 1)); const emit = p.lv ? Math.round(z.poll[p.lv] * A.pollMul * (i.clean ? 0.6 : 1)) : 0;
          head = `<div class="csel"><span class="e">${z.e}</span><div><b>${i.clean ? 'Clean tech' : 'Factory'} · ${p.lv === 0 ? (p.g ? 'under construction' : 'vacant lot') : `level ${p.lv}`}</b><div class="s">${state}</div></div></div>`;
          econ = `<h4>Economy</h4>${kv('Jobs', jobs)}${kv('Levy paid', `${(jobs * 0.1).toFixed(1)} / day`)}${kv('Staffing', `${Math.round(A.staffing * 100)}%`)}`;
          cond = `<h4>Health</h4>${kv('Pollution emitted', emit, '3 tiles around · Health stat thins it')}${kv('Clean tech', i.clean ? 'yes' : `no — city education ${A.education} of 60`)}`;
          if (!i.clean) improve.push(`City education ${A.education} → 60 turns this into clean tech: −40% pollution, +20% jobs.`); if (A.staffing < 0.5) improve.push('Workers short — more homes.');
        }
        return `<div class="rep">${head}${econ}${cond}<h4>How to improve</h4>${li(improve)}</div>`;
      }
      if (SERVICES[p.k]) { const sv = SERVICES[p.k]; const sx = A.svc.find(v => v.x === x && v.y === y); const inReach = Object.values(A.info).filter(o => ZONES[o.p.k] && Math.max(Math.abs(o.x - x), Math.abs(o.y - y)) <= (sx ? sx.R : sv.r)); const homes = inReach.filter(o => o.p.k === 'r').length; const improve = [];
        if (!sx) improve.push('Not next to a road — it does nothing. Bulldoze and rebuild beside a road.'); else if (sx.cap !== Infinity && sx.load > sx.cap) improve.push(`Over capacity (${sx.load}/${sx.cap}) — it works at ${Math.round(sx.eff * 100)}%. Build another${p.k === 'clinic' ? ', or a hospital' : p.k === 'school' ? ', or a university' : ''}.`); else if (sx && inReach.length === 0) improve.push('Nothing zoned in its reach — it costs upkeep for no one.');
        return `<div class="rep"><div class="csel"><span class="e">${sv.e}</span><div><b>${sv.name}</b><div class="s">${WHAT[p.k]}</div></div></div><h4>Stats</h4>${kv('Reach', sx ? sx.R : sv.r, sv.r !== (sx ? sx.R : sv.r) ? 'stretched by your Body stat' : '')}${kv('Zoned tiles in reach', inReach.length, `${homes} homes`)}${sx && sx.cap !== Infinity ? kv('Load', `${sx.load} / ${sx.cap} ${sv.capKind}`, `working at ${Math.round(sx.eff * 100)}%`) : ''}${kv('Upkeep', `${sv.up} Bucks / day`)}${sv.edu ? kv('Education', `+${sv.edu}`, 'for every home in reach') : ''}<h4>How to improve</h4>${li(improve)}</div>`; }
      if (p.k === 'hall') return `<div class="rep"><div class="csel"><span class="e">🏛️</span><div><b>City Hall</b><div class="s">${WHAT.hall}</div></div></div><h4>Stats</h4>${kv('Size', A.mile[1], A.next ? `${A.next[1]} at ${fmt(A.next[0])}` : 'top tier')}${kv('People', fmt(A.pop))}${kv('Happiness', A.happiness)}${kv('Education', A.education)}${kv('Average land value', Math.round(A.avgLV))}${kv('Pollution on homes', Math.round(A.avgPoll))}<div class="chint">Open <b>Report</b> for the full breakdown.</div></div>`;
      if (p.k === 'road') return `<div class="chint">Road${A.conn.has(kkey(x, y)) ? '' : ' — <b>not connected to City Hall</b>'}. ${WHAT.road}</div>`;
      if (DECOR[p.k]) return `<div class="chint"><b>${DECOR[p.k].name}.</b> ${WHAT[p.k]}</div>`;
      return '';
    }
    function renderPanel() {
      const S = get(); const c = S.city; const A = lastA || (lastA = analyze(c, S.vitality)); const peak = A.peak;
      const toolsRow = (list, cols) => `<div class="tools" style="grid-template-columns:repeat(${cols},1fr)">${list.map(([id, e, n]) => `<button class="tool${tool === id ? ' on' : ''}" data-tool="${id}"><span>${e}</span>${n}</button>`).join('')}</div>`;
      let html = `<div class="advisor">🧑‍💼 ${advisor(c, A)}</div><div class="ctabs">${PTABS.map(([id, e, n]) => `<button class="${ptab === id ? 'on' : ''}" data-ptab="${id}">${e} ${n}</button>`).join('')}</div>`;
      if (ptab === 'build') {
        html += toolsRow(TOOLS.filter(t => BUILD_IDS.has(t[0])), 4);
      ...
      if (tool === 'decor') html += `<div class="cgrid">${Object.entries(DECOR).map(([id, d]) => { const rankLock = d.rank && (S.level || 1) < d.rank; const locked = peak < d.mile || rankLock; return `<button class="cbtn decor${sub === id ? ' on' : ''}" data-sub="${id}" ${locked ? 'disabled' : ''}><span class="e">${locked ? '🔒' : d.e}</span><b>${d.name}</b><span class="s">${rankLock ? `Level ${d.rank}` : locked ? `${MILES.find(m => m[0] === d.mile)[1]}` : `${d.bucks} Bucks · +${d.lv} land value${d.rank ? ` · reach ${d.r}` : ''}`}</span></button>`; }).join('')}<button class="cbtn decor" data-fest="1" ${bucks(c) < FEST_BUCKS || c.fest.until > now() ? 'disabled' : ''}><span class="e">🎉</span><b>Festival</b><span class="s">${c.fest.until > now() ? `on · ${Math.ceil((c.fest.until - now()) / HOUR)} h` : `${FEST_BUCKS} Bucks · +5 happiness 24 h`}</span></button></div>`;
        if (!sel) html += `<div class="chint">${tool === 'road' ? `Road · ${ROAD_GRIT} Grit a tile. Tap or drag from City Hall. Two fingers to pan.` : ZONES[tool] ? `${ZONES[tool].name} · ${ZONES[tool].grit} Grit a tile, within 2 tiles of a road. It builds itself over hours once it has power.` : tool === 'bulldoze' ? 'Tap a tile to clear it. No refund.' : tool === 'service' ? 'Pick a service, then tap a tile next to a road. The blue area is its reach.' : tool === 'decor' ? 'Paid in Bucks from taxes. Landmarks unlock by your level.' : ''}</div>`;
      } else if (ptab === 'manage') {
        html += toolsRow(TOOLS.filter(t => MANAGE_IDS.has(t[0])), 3);
      if (tool === 'land') { const r = c.ring.v; const L = landSize(r), N = landSize(r + 1); const gate = LAND_GATE[r] || Infinity; const cost = expandCost(c); const gated = peak < gate; const can = r < 3 && !gated && S.grit >= cost;
        ...
        html += `<div class="crow" style="align-items:center"><label class="s" for="tax">Tax ${c.tax.v}%</label><input type="range" id="tax" min="0" max="20" value="${c.tax.v}" style="flex:1"></div><div class="chint">Tax pays Bucks for decor and festivals. 7% is neutral: every point above costs 3 happiness and cools demand.</div>`; }
      if (tool === 'view') html += `<div class="crow">${[['none', 'Normal'], ['lv', 'Land value'], ['poll', 'Pollution'], ['power', 'Power'], ['water', 'Water'], ['school', 'School'], ['library', 'Library'], ['university', 'University'], ['clinic', 'Clinic'], ['hospital', 'Hospital'], ['fire', 'Fire'], ['police', 'Police'], ['park', 'Parks'], ['bus', 'Bus']].map(([id, n]) => `<button class="btn sm${view === id ? ' primary' : ''}" data-view="${id}">${n}</button>`).join('')}</div>`;
        if (!sel && tool === 'select') html += `<div class="chint">Tap any building for its economy, condition and how to improve it. Drag to pan, pinch or use the buttons to zoom.</div>`;
      } else if (ptab === 'report') html += reportHtml(c, A);
      else html += guideHtml(A, S);
      if (sel && (ptab === 'build' || ptab === 'manage')) html += `<div class="detail">${detailHtml(sel[0], sel[1], c, A)}</div>`;
    ...
    panel.addEventListener('click', e => {
      ...
      const pt = e.target.closest('[data-ptab]'); if (pt) { ptab = pt.dataset.ptab; sub = null; if (ptab === 'manage') tool = 'select'; else if (ptab === 'build') { if (!BUILD_IDS.has(tool)) tool = 'road'; } else tool = ptab; renderPanel(); return; }
      if (tb) { tool = tb.dataset.tool; sub = null; ptab = BUILD_IDS.has(tool) ? 'build' : 'manage'; renderPanel(); return; }
    ...
    function place(x, y, fromDrag) {
      ...
      if (p) { if (!fromDrag) { sel = [x, y]; renderPanel(); } return false; } // occupied → inspect it (not while painting)
    ...
    // a tap on a building's body (which rises above its own tile) picks that building, not the ground behind it
    const HGT = { r: [0, 22, 45, 125], c: [0, 22, 50, 135], i: [0, 22, 30, 42], hall: 40, power: 50, water: 45, school: 30, clinic: 34, fire: 32, police: 32, park: 12, stadium: 40, library: 26, university: 55, hospital: 55, bus: 26, tree: 20, light: 25, plaza: 8, statue: 32, monument: 55, tower: 75, arch: 50, palace: 50 };
    function pickTile(wx, wy, gx, gy) {
      const c = get().city; let best = null, bestD = -1;
      for (const k in c.tiles) { const p = c.tiles[k]; if (p.gone || p.k === 'road') continue; const [x, y] = k.split(',').map(Number); const h = ZONES[p.k] ? HGT[p.k][p.lv] : (HGT[p.k] || 20); if (!h) continue; const [bx, by0] = toScreen(x, y); const base = by0 + THH; if (Math.abs(wx - bx) <= TWH * 0.55 && wy <= base + 2 && wy >= base - h && x + y > bestD) { best = [x, y]; bestD = x + y; } }
      return best || [gx, gy];
    }
    ...
    canvas.addEventListener('pointerdown', e => {
      ...
      const r = canvas.getBoundingClientRect(); const [wx, wy] = toWorld(e.clientX - r.left, e.clientY - r.top); const [gx0, gy0] = toGrid(wx, wy); const [gx, gy] = (tool === 'select' || tool === 'bulldoze') ? pickTile(wx, wy, gx0, gy0) : [gx0, gy0];
    ...
    canvas.addEventListener('pointermove', e => {
      ...
      if (paintTool && drag.moved) { const k = kkey(gx, gy); if (!painted.has(k)) { painted.add(k); if (place(gx, gy, true)) { renderHud(); } } }
    ...
    return { show, refresh: () => { if (visible) { lastA = null; dirtyG = dirtyB = true; renderHud(); if (ptab !== 'report' && ptab !== 'guide') renderPanel(); } },
